...
public class OrderController {
	...
	@ApiOperation(value = "List all orders", httpMethod = "GET", nickname = "listOrders", response = OrderResponse.class, responseContainer = "List", tags = "orders")
	@ApiResponses(value = {
	...
	@ApiOperation(value = "Creates new order", httpMethod = "POST", nickname = "createOrder", tags = "orders")
	@ApiResponses(value = { @ApiResponse(code = 201, message = "Item created"),
			@ApiResponse(code = 400, message = "Invalid input, object invalid"),
			@ApiResponse(code = 404, message = "Burger ID not found"),
			@ApiResponse(code = 500, message = "Internal system error") })
	@RequestMapping(value = ORDERS_URI, method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON, produces = MediaType.APPLICATION_JSON)
	public ResponseEntity<Void> create(@ApiParam(value = "Order to create") @Valid @RequestBody OrderRequest order)
			throws NotFoundException, BadRequestAPIException {
		OrderResponse saved = orderService.save(order);

		UriComponents resourceUri = ServletUriComponentsBuilder.fromCurrentRequest().path("/{orderId}")
				.buildAndExpand(saved.getId());

		return ResponseEntity.created(resourceUri.toUri()).build();
	}

	@ApiOperation(value = "List all data about a order", httpMethod = "GET", nickname = "getOrder", response = OrderResponse.class, tags = "orders")
	@ApiResponses(value = { @ApiResponse(code = 200, message = "A single order", response = OrderResponse.class),
			@ApiResponse(code = 404, message = "Order with this ID not found") })
	@RequestMapping(value = ORDERS_URI
			+ "/{orderId}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON)
	public ResponseEntity<OrderResponse> fetchOrder(
			@ApiParam(value = "ID of order that needs to be fetched", required = true) @PathVariable("orderId") String id)
			throws NotFoundException {
		return ResponseEntity.ok(orderService.fetch(id));
	}

	@ApiOperation(value = "Replaces a order", httpMethod = "PUT", nickname = "replaceOrder", tags = "orders")
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Item replaced"),
			...
			@ApiResponse(code = 500, message = "Internal system error") })
	@RequestMapping(value = ORDERS_URI
			+ "/{orderId}", method = RequestMethod.PUT, consumes = MediaType.APPLICATION_JSON)
	public ResponseEntity<Void> replace(
			...
			@ApiParam(value = "Order to replace") @Valid @RequestBody OrderRequest orderRequest)
			throws NotFoundException {
	...
	public ResponseEntity<Void> deleteIngredient(
			@ApiParam(value = "ID of order that needs to be deleted", required = true) @PathVariable("orderId") String id)
			throws NotFoundException {
