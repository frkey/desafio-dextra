...
public class IngredientController {
	...
	@ApiOperation(value = "List all ingredients", httpMethod = "GET", nickname = "listIngredients", response = IngredientResponse.class, responseContainer = "List", tags = "ingredients")
	@ApiResponses(value = {
	...
	@ApiOperation(value = "Creates new ingredient", httpMethod = "POST", nickname = "createIngredient", tags = "ingredients")
	@ApiResponses(value = { @ApiResponse(code = 201, message = "Item created"),
			@ApiResponse(code = 400, message = "Invalid input, object invalid"),
			@ApiResponse(code = 500, message = "Internal system error") })
	@RequestMapping(value = INGREDIENTS_URI, method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON, produces = MediaType.APPLICATION_JSON)
	public ResponseEntity<Void> create(
		...
		UriComponents resourceUri = ServletUriComponentsBuilder.fromCurrentRequest().path("/{ingredientId}")
				.buildAndExpand(saved.getId());

		return ResponseEntity.created(resourceUri.toUri()).build();
	}

	@ApiOperation(value = "List all data about a ingredient", httpMethod = "GET", nickname = "getIngredient", response = IngredientResponse.class, tags = "ingredients")
	@ApiResponses(value = {
	...
	@RequestMapping(value = INGREDIENTS_URI
			+ "/{ingredientId}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON)
	public ResponseEntity<IngredientResponse> fetchIngredient(
			@ApiParam(value = "ID of ingredient that needs to be fetched", required = true) @PathVariable("ingredientId") String id)
	...
	public ResponseEntity<Void> deleteIngredient(
			@ApiParam(value = "ID of ingredient that needs to be deleted", required = true) @PathVariable("ingredientId") String id)
			throws NotFoundException {
